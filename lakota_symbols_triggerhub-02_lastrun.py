...
expName = 'lakota_symbols_triggerhub-02'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\skite\\Documents\\Github\\lakota-neuro-exp\\lakota_symbols_triggerhub-02_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    instruct = visual.TextStim(win=win, name='instruct',
        text='You will see a series of visual designs (symbols).\n\nYour task is simply to look carefully at each design while keeping your eyes on the screen and staying as still as possible — try not to move your eyes, face, or hands, and keep blinking to a minimum.\n\nThe experiment has a few short parts, and a brief instruction screen will appear before each one:\n\n1. Rest — relax and look at the fixation cross for about 1 minute.\n\n2. Quick viewing — each design appears for about 1 second and is shown 2 times.\n\n3. Longer viewing — each design is shown once for about 30 seconds. After each one, you will rate how meaningful it feels to you.\n\n4. Words — each design is shown once more, and you write 10 words it brings to mind.\n\n5. Rest — a final 1-minute rest period.\n\nThere are 20 designs in total, each shown 4 times across the experiment (2 quick + 1 long + 1 words), in random order.\n\nPlease stay relaxed and as still as possible throughout.\n\nWhen you are ready to begin, press the SPACE BAR to start.',
        font='Arial',
    ...
    def send_trigger_phasic(ser, trigger_value, dur=0.05):
    ...
    # --- Initialize components for Routine "resting_state_start" ---
    key_resp_3 = keyboard.Keyboard(deviceName='key_resp_3')
    text_3 = visual.TextStim(win=win, name='text_3',
        text='Time to rest.\n\nPlease sit still and keep your eyes on the fixation cross for the next minute.\n\nTry to relax, and keep blinking to a minimum.\n\nWhen you are ready, press the SPACE BAR to begin.',
        font='Arial',
    ...
    fixation_cross = visual.ImageStim(
        win=win,
        name='fixation_cross', 
        image='stimuli/fixation_cross_old.jpg', mask=None, anchor='center',
    ...
    text_2 = visual.TextStim(win=win, name='text_2',
        text='We will now begin the first task.\n\nA series of designs will appear briefly on the screen, one at a time.\n\nSimply look carefully at each design while keeping your eyes on the screen and staying as still as possible.\n\nWhen you are ready, press the SPACE BAR to start.',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    # --- Initialize components for Routine "instruction_part2" ---
    text_4 = visual.TextStim(win=win, name='text_4',
        text='Part 2\n\nThis time, each design will stay on the screen for longer.\n\nRather than just looking, let yourself take it in. Focus on what the symbol means to you, allow yourself to become absorbed in it, and notice any images, feelings, or associations it brings to mind.\n\nAfter each design, you will be asked how meaningful it felt to you. There are no right or wrong answers — simply respond with your honest impression.\n\nTake your time and stay with each design.\n\nWhen you are ready, press the SPACE BAR to continue.',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    # --- Initialize components for Routine "empty_gray_3" ---
    empty_gray_4 = visual.ImageStim(
        win=win,
        name='empty_gray_4', 
        image='stimuli/fixation_cross.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), draggable=False, size=(0.5, 0.5),
    ...
    # --- Prepare to start Routine "resting_state_start" ---
    # create an object to store info about Routine resting_state_start
    resting_state_start = data.Routine(
        name='resting_state_start',
        components=[key_resp_3, text_3],
    )
    resting_state_start.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    ...
    key_resp_3.rt = []
    _key_resp_3_allKeys = []
    # store start times for resting_state_start
    resting_state_start.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    resting_state_start.tStart = globalClock.getTime(format='float')
    resting_state_start.status = STARTED
    thisExp.addData('resting_state_start.started', resting_state_start.tStart)
    resting_state_start.maxDuration = None
    # keep track of which components have finished
    resting_state_startComponents = resting_state_start.components
    for thisComponent in resting_state_start.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "resting_state_start" ---
    resting_state_start.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        if thisExp.status == PAUSED:
            pauseExperiment(
                ...
                win=win, 
                timers=[routineTimer, globalClock], 
                currentRoutine=resting_state_start,
            )
            # skip the frame we paused on
            continue
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            resting_state_start.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in resting_state_start.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    # --- Ending Routine "resting_state_start" ---
    for thisComponent in resting_state_start.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for resting_state_start
    resting_state_start.tStop = globalClock.getTime(format='float')
    resting_state_start.tStopRefresh = tThisFlipGlobal
    thisExp.addData('resting_state_start.stopped', resting_state_start.tStop)
    # check responses
    ...
    thisExp.nextEntry()
    # the Routine "resting_state_start" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # --- Prepare to start Routine "resting_state" ---
    # create an object to store info about Routine resting_state
    resting_state = data.Routine(
        name='resting_state',
        components=[fixation_cross],
    )
    resting_state.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # Run 'Begin Routine' code from code_5
    win.callOnFlip(send_trigger_tonic, ser, RESTING_STATE)
    # store start times for resting_state
    resting_state.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    resting_state.tStart = globalClock.getTime(format='float')
    resting_state.status = STARTED
    thisExp.addData('resting_state.started', resting_state.tStart)
    resting_state.maxDuration = None
    # keep track of which components have finished
    resting_stateComponents = resting_state.components
    for thisComponent in resting_state.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    resting_state.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine and routineTimer.getTime() < 2.0:
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            resting_state.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in resting_state.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    for thisComponent in resting_state.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for resting_state
    resting_state.tStop = globalClock.getTime(format='float')
    resting_state.tStopRefresh = tThisFlipGlobal
    thisExp.addData('resting_state.stopped', resting_state.tStop)
    # Run 'End Routine' code from code_5
    reset_trigger_tonic(ser)
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    if resting_state.maxDurationReached:
        routineTimer.addTime(-resting_state.maxDuration)
    elif resting_state.forceEnded:
        routineTimer.reset()
    else:
    ...
    for thisTrial in trials:
        ...
        while continueRoutine and routineTimer.getTime() < 0.5:
            ...
            if empty_gray.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > empty_gray.tStartRefresh + 0.5-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        # pick whatever code you want here (constant or based on the image)
        
        
        # send exactly on the flip that will show the first frame of this routine
        win.callOnFlip(send_trigger_tonic, ser, IMAGE_ONSET)
    ...
    # --- Prepare to start Routine "instruction_part2" ---
    # create an object to store info about Routine instruction_part2
    instruction_part2 = data.Routine(
        name='instruction_part2',
        components=[text_4, key_resp_4],
    )
    instruction_part2.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    ...
    key_resp_4.rt = []
    _key_resp_4_allKeys = []
    # store start times for instruction_part2
    instruction_part2.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    instruction_part2.tStart = globalClock.getTime(format='float')
    instruction_part2.status = STARTED
    thisExp.addData('instruction_part2.started', instruction_part2.tStart)
    instruction_part2.maxDuration = None
    # keep track of which components have finished
    instruction_part2Components = instruction_part2.components
    for thisComponent in instruction_part2.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "instruction_part2" ---
    instruction_part2.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        if thisExp.status == PAUSED:
            pauseExperiment(
                ...
                win=win, 
                timers=[routineTimer, globalClock], 
                currentRoutine=instruction_part2,
            )
            # skip the frame we paused on
            continue
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            instruction_part2.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in instruction_part2.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    # --- Ending Routine "instruction_part2" ---
    for thisComponent in instruction_part2.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for instruction_part2
    instruction_part2.tStop = globalClock.getTime(format='float')
    instruction_part2.tStopRefresh = tThisFlipGlobal
    thisExp.addData('instruction_part2.stopped', instruction_part2.tStop)
    # check responses
    ...
    thisExp.nextEntry()
    # the Routine "instruction_part2" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    for thisTrial_3 in trials_3:
        ...
        # --- Prepare to start Routine "empty_gray_3" ---
        # create an object to store info about Routine empty_gray_3
        empty_gray_3 = data.Routine(
            name='empty_gray_3',
            components=[empty_gray_4],
        )
        empty_gray_3.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        ...
        # store start times for empty_gray_3
        empty_gray_3.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        empty_gray_3.tStart = globalClock.getTime(format='float')
        empty_gray_3.status = STARTED
        thisExp.addData('empty_gray_3.started', empty_gray_3.tStart)
        empty_gray_3.maxDuration = None
        # keep track of which components have finished
        empty_gray_3Components = empty_gray_3.components
        for thisComponent in empty_gray_3.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "empty_gray_3" ---
        empty_gray_3.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine and routineTimer.getTime() < 1.0:
            # if trial has changed, end Routine now
            ...
            # *empty_gray_4* updates
            
            # if empty_gray_4 is starting this frame...
            if empty_gray_4.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                empty_gray_4.frameNStart = frameN  # exact frame index
                empty_gray_4.tStart = t  # local t and not account for scr refresh
                empty_gray_4.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(empty_gray_4, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'empty_gray_4.started')
                # update status
                empty_gray_4.status = STARTED
                empty_gray_4.setAutoDraw(True)
            
            # if empty_gray_4 is active this frame...
            if empty_gray_4.status == STARTED:
                # update params
                pass
            
            # if empty_gray_4 is stopping this frame...
            if empty_gray_4.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > empty_gray_4.tStartRefresh + 1-frameTolerance:
                    # keep track of stop time/frame for later
                    empty_gray_4.tStop = t  # not accounting for scr refresh
                    empty_gray_4.tStopRefresh = tThisFlipGlobal  # on global time
                    empty_gray_4.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'empty_gray_4.stopped')
                    # update status
                    empty_gray_4.status = FINISHED
                    empty_gray_4.setAutoDraw(False)
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                empty_gray_3.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in empty_gray_3.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        # --- Ending Routine "empty_gray_3" ---
        for thisComponent in empty_gray_3.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for empty_gray_3
        empty_gray_3.tStop = globalClock.getTime(format='float')
        empty_gray_3.tStopRefresh = tThisFlipGlobal
        thisExp.addData('empty_gray_3.stopped', empty_gray_3.tStop)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if empty_gray_3.maxDurationReached:
            routineTimer.addTime(-empty_gray_3.maxDuration)
        elif empty_gray_3.forceEnded:
            routineTimer.reset()
        else:
            routineTimer.addTime(-1.000000)
        ...
        # send exactly on the flip that will show the first frame of this routine
        win.callOnFlip(send_trigger_tonic, ser, IMAGE_LONG_ONSET)
        
        
        image_4.setImage(image)
        ...
        current_img = image_4.image  # same variable used in the trial routine
    ...
    for thisTrial_2 in trials_2:
        ...
        win.callOnFlip(send_trigger_tonic, ser, IMAGE_WORDS)
