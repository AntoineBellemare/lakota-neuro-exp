...
expName = 'lakota_symbols_lsl'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\skite\\Documents\\Github\\lakota-neuro-exp\\lakota_symbols_lsl_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # Create an LSL stream for markers
    marker_stream_name = "PsychoPyMarkers"   # keep this name for lsl-tools
    info = StreamInfo(
        marker_stream_name,
        "Markers",
        1,          # one channel: the marker value
        0,          # 0 = irregular event stream
        "string",   # or "int32" if you prefer pure integers
        "psychopy_marker_source"
    )
    
    marker_outlet = StreamOutlet(info)
    
    def send_marker(msg: str):
        """Send one marker via LSL."""
        try:
            marker_outlet.push_sample([msg])
            print("LSL SENT:", msg)
        except Exception as e:
            print("LSL ERROR:", e)
    
    print("✅ LSL marker stream created:", marker_stream_name)
    ...
    for thisTrial in trials:
        ...
        # Send ON marker exactly on the flip that draws the image
